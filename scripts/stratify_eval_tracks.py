...
def resolve_input_path(primary: Path, fallbacks):
    if primary.exists():
        return primary
    for fb in fallbacks:
        if fb.exists():
...
def main():
    ...
    ap.add_argument("--input", default="analysis/llm_selection/splits_eval.json")
    ap.add_argument("--output", default="analysis/human_evaluation/eval_for_human_tagging.json")
    ...
    fallback = Path("analysis/llm_selection/splits/eval.json")
    in_path = resolve_input_path(primary, [fallback])
    rows = load_json_array(in_path)
