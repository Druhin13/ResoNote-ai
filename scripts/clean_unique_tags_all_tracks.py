...
def generate_morph_noun_candidates(word: str) -> List[str]:
    ...
    if word.endswith("ive"):
        c.add(word[:-3] + "ion"); c.add(word + "ness")
    if word.endswith("al"):
        c.add(word[:-2] + "ality"); c.add(word[:-2] + "ation")
    if word.endswith("ic") and len(word) > 2:
        c.add(word[:-2]); c.add(word[:-2] + "ics")
    if word.endswith("ous"):
        c.add(word[:-3] + "ity"); c.add(word[:-3] + "ousness")
    ...
    if word.endswith("ing") and len(word) > 4:
        c.add(word[:-3]); c.add(word[:-3] + "ingness")
    return sorted(c)
...
def process(src: Path, dst: Path) -> Path:
    try:
        ...
        raise RuntimeError("This script requires `inflect`. Install with: pip install inflect nltk")
...
def main():
    ap = argparse.ArgumentParser(description="Clean unique tags per facet; adjectives for Emotional_Tone; nouns for others.")
    ap.add_argument("--src", type=Path, default=Path("analysis/llm_tagging/all_tracks_unique_tags.json"))
    ap.add_argument("--dst", type=Path, default=Path("analysis/llm_tagging/all_tracks_unique_tags_cleaned.json"))
    args = ap.parse_args()
